...
# 2. APP & MODEL LOADING
# =============================================================================
app = FastAPI(title="Digital Twin API (Live Headless Edition)")
device = torch.device("cpu")  # Using CPU for low-latency live inference
MDL = "models"

print("🧠 Booting Headless AI Architecture...")
try:
...
# 3. DOCKER ACTUATOR
# =============================================================================
def block_ip_in_docker(attacker_ip):
    try:
        cmd = f'docker exec core_switch sh -c "ovs-ofctl del-flows br-core ip,nw_src={attacker_ip} && ovs-ofctl add-flow br-core priority=100,ip,nw_src={attacker_ip},actions=drop"'
        subprocess.run(cmd, shell=True, check=True, capture_output=True)
        print(f"🛑 [ACTUATOR] Successfully severed {attacker_ip} at the Core Switch.")
    except Exception as e:
        print(f"❌ [ACTUATOR] Failed to block {attacker_ip}: {e}")
...
async def live_network_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # 1. Map Live Features to the 40-Dimension Legacy Array
            ...
            # Map Scapy extractions to ISCX equivalents
            legacy_input[0, 0] = q_latency                           # Flow Duration Proxy
            legacy_input[0, 1] = q_volume                            # Total Fwd Pkts Proxy
            legacy_input[0, 5] = payload.get("max_packet_length", 0) # Max Pkt Len
            legacy_input[0, 6] = payload.get("min_packet_length", 0) # Min Pkt Len
            legacy_input[0, 13] = payload.get("throughput", 0)       # Flow Bytes/s
            legacy_input[0, 30] = payload.get("fin_count", 0)        # FIN
            legacy_input[0, 31] = payload.get("syn_count", 0)        # SYN
            legacy_input[0, 32] = payload.get("rst_count", 0)        # RST
            legacy_input[0, 33] = payload.get("psh_count", 0)        # PSH
            legacy_input[0, 34] = payload.get("ack_count", 0)        # ACK
            legacy_input[0, 35] = payload.get("urg_count", 0)        # URG
            legacy_input[0, 37] = payload.get("avg_packet_size", 0)  # Avg Pkt Size

            # Scale and buffer inputs
            ...
            if len(seq_buf) < 10:
                continue # Wait for sequence buffer to fill
            ...
            # --- 3. ML DEBOUNCING & CONFIDENCE MATRICES ---
            # Adaptive Threshold
            ...
            # --- 4. RL MANAGER DECISION ---
            state = np.array([[
                q_latency,           # Primary Latency
                0.05,                # Backup Latency (Static proxy for live)
                mae,                 # Observer Error
                base_conf,           # Unified Confidence
                traffic_type,        # Analyst Cluster Type
                float(last_action)   # Hysteresis
            ]], dtype=np.float32)
            ...
            if last_action == 1 and src_ip not in blocked_ips and src_ip != "10.0.0.2":
                print(f"⚠️ [AI DETECT] Anomaly from {src_ip} | Conf: {sustained_confidence:.2f} | Lat: {q_latency:.3f}s")
                if sustained_confidence > 0.35 or (sig_lat > 0.5 and max(conf_buf) > 0.28):
                    blocked_ips.add(src_ip)
                    asyncio.create_task(asyncio.to_thread(block_ip_in_docker, src_ip))
